...
# 经纬度数据，baidu转wgs84
def points_transform_baidu2wgs(geo_AOI):
    points_result = []
    geo_AOI = geo_AOI.split('|')
    point_blocks = re.split(';', geo_AOI[2])
    for point_b in point_blocks:
        points_all = [] 
        point = point_b.split(",")
        point_transform = []
        for i in range(int(len(point)/2)):
            #第一个点的x坐标删除‘1-’
            if i==0 and point[0][1:2]=='-':
                point[2*i] = point[2*i][2:]
            elif i==0 and point[0][2:3]=='-':
                point[2*i] = point[2*i][3:]
            elif i==0 and point[0][3:4]=='-':
                point[2*i] = point[2*i][4:]
            #最后的点的y坐标删除‘;’
            if i==int((len(point)/2)-1):
                point[2*i+1] = point[2*i+1][:-1]
            point_Mecator2BD09 = mercatortobd09(float(point[2*i]),float(point[2*i+1]))
            lon=str(point_Mecator2BD09[0])[:10]
            lat=str(point_Mecator2BD09[1])[:9]
            point_gcj02 = bd09_to_gcj02(float(lon), float(lat))
            point_wgs84 = gcj02_to_wgs84(point_gcj02[0], point_gcj02[1])
            point_transform.append([round(point_wgs84[0],6), round(point_wgs84[1],6)])
        point_str = ''
        for j in range(len(point_transform)):
            point_str = (str(point_transform[j])).replace(' ','')[1:-1]
            points_all.append(point_str)
        points_result.append(points_all)
    return points_result


#print(mercatortobd09(12741389.00,4557257.00))
